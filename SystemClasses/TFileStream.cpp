	...
	: TWrapperStream(), filename(FileName)
{
	...
	_stream = std::make_shared<fs::fstream>(fs::path(filename), mode);
	reopen(_stream);
	m_position = 0;
}

TFileStream::TFileStream(const fs::path &path, const uint16_t fileMode)
	: TWrapperStream(), filename(path.string())
{
	...
	_stream = std::make_shared<fs::fstream>(path, mode);
	reopen(_stream);
	m_position = 0;
}
